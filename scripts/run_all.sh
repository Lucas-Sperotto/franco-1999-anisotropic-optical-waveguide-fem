...
Run the implemented reproduction sweeps:
  ...
  - Case 4: Gaussian-Gaussian diffused channel
  - Case 5: APE LiNbO3 anisotropic guide
  - Case 6: Ti:LiNbO3 anisotropic guide
...
if [[ "${SMOKE}" -eq 1 ]]; then
    ...
    CASE4_ROOT="${REPO_ROOT}/build/test_output/run_all_smoke/case4_gaussian_gaussian"
    ...
    CASE4_ROOT="${REPO_ROOT}/out/case4_gaussian_gaussian/final_run"
    CASE5_ROOT="${REPO_ROOT}/out/case5_ape_linbo3/final_run"
    CASE6_ROOT="${REPO_ROOT}/out/case6_ti_linbo3/final_run"
    SWEEP_MODE_ARGS=()
fi
...
python3 "${SCRIPT_DIR}/run_case4_gaussian_gaussian_sweep.py" \
    --skip-build \
    "${SWEEP_MODE_ARGS[@]}" \
    --output-root "${CASE4_ROOT}"

echo "  Case 5 -> ${CASE5_ROOT}"
python3 "${SCRIPT_DIR}/run_case5_ape_linbo3_sweep.py" \
    --skip-build \
    "${SWEEP_MODE_ARGS[@]}" \
    --output-root "${CASE5_ROOT}"

echo "  Case 6 -> ${CASE6_ROOT}"
python3 "${SCRIPT_DIR}/run_case6_ti_linbo3_sweep.py" \
    --skip-build \
    "${SWEEP_MODE_ARGS[@]}" \
    --output-root "${CASE6_ROOT}"
