#!/usr/bin/env python3
"""Generate SVG dispersion-curve plot for Case 4 (Gaussian-Gaussian, Fig. 5).

Reads:  <sweep-root>/consolidated/dispersion_curve.csv
Writes: <sweep-root>/plots/fig5_like_reference.svg

Plot mirrors Fig. 5 of Franco et al. (1999):
  X-axis: normalised frequency V = (k0*b/pi)*sqrt(n3m^2 - n2^2)
  Y-axis: normalised propagation constant B = (neff^2 - n2^2)/(n3m^2 - n2^2)
  Multiple mode curves plotted.
"""
...
MODE_LABELS = ["E11 (modo 1)", "E12 / E21 (modo 2)", "modo 3", "modo 4"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a B-vs-V dispersion-curve SVG for Case 4."
    )
    parser.add_argument("--sweep-root", required=True)
    parser.add_argument("--output-name", default="fig5_like_reference.svg")
...
def main() -> None:
    ...
    if not dispersion_csv.exists():
        raise FileNotFoundError(
            ...
            "Run consolidate_case4_gaussian_gaussian_sweep.py first."
        )
    ...
    for row in rows:
        ...
        if not (math.isfinite(v_val) and math.isfinite(b_val)):
            continue
        if FIG5_V_MIN <= v_val <= FIG5_V_MAX and FIG5_B_MIN <= b_val <= FIG5_B_MAX:
    ...
    x_ticks = build_even_ticks(FIG5_V_MIN, FIG5_V_MAX, 11)
    y_ticks = build_even_ticks(FIG5_B_MIN, FIG5_B_MAX, 6)
    ...
    svg_parts = [
        ...
        f'Caso 4 - Guia de canal difuso isotrópico (perfil Gaussian-Gaussian)</text>',
        f'<text x="{width / 2:.1f}" y="46" text-anchor="middle" '
        f'font-size="13" fill="#555" font-family="Arial">'
        f'n1=1.00, n2=&#x221A;2.1, n3m=1.05&#xB7;n2, a=b=1.0 µm, delta_x/delta_z: ON'
        f'</text>',
    ...
    for x_tick in x_ticks:
        x_px = map_value(x_tick, FIG5_V_MIN, FIG5_V_MAX, plot_left, plot_right)
        svg_parts.extend([
    ...
    for y_tick in y_ticks:
        y_px = map_value(y_tick, FIG5_B_MIN, FIG5_B_MAX, plot_bottom, plot_top)
        svg_parts.extend([
    ...
    for i, (mode_idx, pts) in enumerate(sorted(modes_data.items())):
        ...
        pixel_points = [
            (
                map_value(v, FIG5_V_MIN, FIG5_V_MAX, plot_left, plot_right),
                map_value(b, FIG5_B_MIN, FIG5_B_MAX, plot_bottom, plot_top),
            )
            for v, b in pts
        ]
    ...
    svg_parts.append(
        ...
        f'Caso 4: perfil Gaussian-Gaussian, delta_x/delta_z ON. '
