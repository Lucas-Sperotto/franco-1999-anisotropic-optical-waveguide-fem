...
O arquivo [case5_ape_linbo3.yaml](case5_ape_linbo3.yaml) implementa o caso-base do **Caso 5**. Ele usa a Eq. 10 de `docs/06` para relacionar concentração normalizada e índice extraordinário, e o sweep da Fig. 6 usa `scripts/ape_diffusion_preprocessor.py` para calcular uma aproximação Gaussian-Gaussian a partir das constantes de difusão do APE. A limitação restante é que essa aproximação ainda não substitui a solução FEM 2D de difusão anisotrópica descrita no artigo.

O arquivo [case6_ti_linbo3.yaml](case6_ti_linbo3.yaml) implementa o caso-base do **Caso 6**. Ele usa as Eqs. 11-12 de `docs/06` com parâmetros separados para os ramos extraordinário e ordinário. O sweep em `W` e a extração de `W_x/W_y` são automatizados por `scripts/run_case6_ti_linbo3_sweep.py` e `scripts/consolidate_case6_ti_linbo3_sweep.py`.
